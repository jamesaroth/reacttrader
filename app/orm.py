...
class ORM:
    ...
    @classmethod
    def select_many_orderby(cls, where_clause="", orderby="", values=tuple()):
        """ provide a WHERE and ORDER BY clause to a SELECT statement and return objects
        representing each matched row """
        with sqlite3.connect(cls.database) as conn:
            conn.row_factory = sqlite3.Row
            cur = conn.cursor()
            SQLPATTERN = "SELECT * FROM {table} {where_clause} {orderby};"
            SQL = SQLPATTERN.format(table=cls.table, where_clause=where_clause, orderby=orderby)
            cur.execute(SQL, values)
            result = []
            for row in cur.fetchall():
                result.append(cls._from_row(row))
